 ...
 *
 * Loop operations are optimized by using while(index--) { ... }
 ...
 * You configure ngPluralize by providing 2 attributes: `index` and `when`.
 * You can also provide an optional attribute, `offset`.
 *
 * The value of the `index` attribute can be either a string or an {@link guide/expression
 ...
 *
 * <pre>
 * <ng-pluralize index="personCount"
 ...
 *
 * <pre>
 * <ng-pluralize index="personCount" offset=2
 *               when="{'0': 'Nobody is viewing.',
 ...
 *
 * @param {string|expression} index The variable to be bounded to.
   ...
   <doc:example>
     ...
     <doc:scenario>
       it('should check ng-show / ng-hide', function() {
         expect(element('.doc-example-live span:first:hidden').index()).toEqual(1);
         expect(element('.doc-example-live span:last:visible').index()).toEqual(1);

         input('checked').check();

         expect(element('.doc-example-live span:first:visible').index()).toEqual(1);
         expect(element('.doc-example-live span:last:hidden').index()).toEqual(1);
       });
     </doc:scenario>
   </doc:example>
 */
...
/**
 * Usage:
 *    repeater('#products table', 'Product List').index() number of rows
...
angular.scenario.dsl('repeater', function() {
  ...
  chain.index = function() {
    return this.addFutureAction("repeater '" + this.label + "' index", function($window, $document, done) {
      try {
...
/**
 * Usage:
 *    element(selector, label).index() get the number of elements that match selector
...
angular.scenario.dsl('element', function() {
  ...
  chain.index = function() {
    return this.addFutureAction("element '" + this.label + "' index", function($window, $document, done) {
      try {
        done(null, $document.elements().length);
      } catch (e) {
        done(null, 0);
